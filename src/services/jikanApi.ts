...
export async function getSeasonalAnime(year?: number, season?: string, page: number = 1): Promise<AnimeResponse> {
  let url = `${BASE_URL}/seasons/${year || 2024}/${season || 'winter'}?page=${page}`
  const response = await fetch(url)
  if (!response.ok) throw new Error('Failed to fetch seasonal anime')
  return response.json()
}

export async function searchAnime(query: string, page: number = 1): Promise<AnimeResponse> {
  const response = await fetch(`${BASE_URL}/anime?q=${encodeURIComponent(query)}&page=${page}&limit=24`)
...
export async function getUpcomingAnime(page: number = 1): Promise<AnimeResponse> {
